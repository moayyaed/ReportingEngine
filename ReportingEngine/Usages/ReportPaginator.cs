...
namespace ReportingEngine.Usages
{
    ...
    public class ReportPaginator : DocumentPaginator
    {
        ...
        public ReportPaginator(ReportDocument report, ReportData data)
        {
            ...
            ArrayList listPageHeaders = _dynamicCache.GetFlowDocumentVisualListByType(typeof(ReportHeader));
            if (listPageHeaders.Count > 1) throw new ArgumentException("Flow document can have only one report header section");
            if (listPageHeaders.Count == 1) _blockPageHeader = (ReportHeader)listPageHeaders[0];
            ArrayList listPageFooters = _dynamicCache.GetFlowDocumentVisualListByType(typeof(ReportFooter));
            if (listPageFooters.Count > 1) throw new ArgumentException("Flow document can have only one report footer section");
            if (listPageFooters.Count == 1) _blockPageFooter = (ReportFooter)listPageFooters[0];
